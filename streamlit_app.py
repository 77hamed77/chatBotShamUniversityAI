...
try:
    ...
    if not google_api_key:
        st.error("❌ خطأ: متغير البيئة 'GOOGLE_API_KEY' غير موجود في ملف .env. يرجى تعيينه لتشغيل الذكاء الاصطناعي التوليدي.")
        st.stop()
    llm = ChatGoogleGenerativeAI(model="gemini-1.5-flash", google_api_key=google_api_key, temperature=0) # استخدام نموذج Google Gemini
    ...
    if not serper_api_key:
        st.warning("⚠️ تحذير: متغير البيئة 'SERPER_API_KEY' غير موجود في ملف .env. لن يتم تفعيل أداة البحث على الويب.")
        serper_search = None
    else:
        serper_search = GoogleSerperAPIWrapper(serper_api_key=serper_api_key, gl="sa", hl="ar", k=3) # البحث باللغة العربية، أول 3 نتائج
